 ...
 */
@WebFilter( urlPatterns = { "/jsp/limited/*" })
public class LimitAccessFilter extends AbstractFilter {
    @Override
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session == null || session.getAttribute("user") == null || (session.getAttribute("user") != null &&
                !"user".equals(((User) session.getAttribute("user")).getRole().getValue()) &&
                !"administrator".equals(((User) session.getAttribute("user")).getRole().getValue()) &&
                !"expert".equals(((User) session.getAttribute("user")).getRole().getValue()))) {
